var hello, hey, textEntered, target; // Declare variables

hey = document.getElementById('hey'); // Element that holds note
hello = document.getElementById('hello'); // Input for writing the note
...
function writeLabel(e) { // Declare function
    ...
    hey.textContent = textEntered; // Update note text
}
...
if (document.addEventListener) { // If event listener supported
    ...
    hello.addEventListener('input', writeLabel);
} else { // Otherwise
    ...
    hello.attachEvent('onkeyup', writeLabel);
}
