var nathan, joe, textEntered, target; // Declare variables

joe = document.getElementById('joe'); // Element that holds note
nathan = document.getElementById('nathan'); // Input for writing the note
...
function writeLabel(e) { // Declare function
    ...
    joe.textContent = textEntered; // Update note text
}
...
if (document.addEventListener) { // If event listener supported
    ...
    nathan.addEventListener('input', writeLabel);
} else { // Otherwise
    ...
    nathan.attachEvent('onkeyup', writeLabel);
}
